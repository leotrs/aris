...
test.describe('Mobile Responsiveness E2E', () => {
  ...
  test.describe('Responsive Layout', () => {
    mobileViewports.forEach(viewport => {
      test(`should render correctly on ${viewport.name}`, async ({ page }) => {
        ...
        await expect(page.locator('.section-three')).toBeVisible();
        await expect(page.locator('.section-four')).toBeVisible();
      });
    });
  ...
  test.describe('Touch Interactions', () => {
    ...
    test('should handle scroll behavior on mobile', async ({ page }) => {
      ...
      await page.locator('.section-three').scrollIntoViewIfNeeded();
      await expect(page.locator('.section-three')).toBeInViewport();

      await page.locator('footer').scrollIntoViewIfNeeded();
      await expect(page.locator('footer')).toBeInViewport();

      // Test back to top functionality
      await page.tap('.back-to-top');
