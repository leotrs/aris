...
print(f"🔍 [RESET-USER-DEBUG] Starting test user reset script")
...
print(f"🔍 [RESET-USER-DEBUG] Environment variables check:")
...
async def reset_test_user():
    ...
    print(f"🔍 [RESET-USER-DEBUG] Initializing database session...")
    
    try:
        session = ArisSession()
        print(f"✅ [RESET-USER-DEBUG] Database session created successfully")
    ...
    try:
        ...
        if user_row:
            ...
            print(f"🔍 [RESET-USER-DEBUG] Committing user update...")
            await session.commit()
            print(f"✅ [RESET-USER-DEBUG] Existing user updated successfully")
        else:
            print(f"🔍 [RESET-USER-DEBUG] No existing user found, creating new user...")
            # Create new test user
            password_hash = hash_password(TEST_USER_PASSWORD)
            print(f"🔍 [RESET-USER-DEBUG] Password hash created (length: {len(password_hash)})")
            user = User(name="Test User", email=TEST_USER_EMAIL, password_hash=password_hash)
            session.add(user)
            print(f"🔍 [RESET-USER-DEBUG] User added to session, committing...")
            await session.commit()
            user_id = user.id
...
::""",
        ...
        print(f"🔍 [RESET-USER-DEBUG] Committing test files...")
        await session.commit()
        print(f"✅ [RESET-USER-DEBUG] Created {len(test_files)} test files")

        # Create stable test tags
        print(f"🔍 [RESET-USER-DEBUG] Creating test tags...")
        test_tags = [
        ...
        print(f"🔍 [RESET-USER-DEBUG] Committing test tags...")
        await session.commit()
        ...
        # Final verification
        print(f"🔍 [RESET-USER-DEBUG] Final verification - querying user again...")
        ...
        if verify_row:
            ...
            print(f"❌ [RESET-USER-DEBUG] Verification FAILED - user not found after creation!")

    except Exception as e:
        print(f"❌ [RESET-USER-DEBUG] Exception occurred: {e}")
        print(f"❌ [RESET-USER-DEBUG] Exception type: {type(e).__name__}")
        print(f"❌ [RESET-USER-DEBUG] Rolling back transaction...")
        await session.rollback()
        print(f"❌ Error resetting test user: {e}")
        raise
    finally:
        print(f"🔍 [RESET-USER-DEBUG] Closing database session...")
        await session.close()
        print(f"🔍 [RESET-USER-DEBUG] Session closed")
