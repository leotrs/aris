...
from aris.models import User, Tag, File, file_tags
...
async def test_create_tag_success(db_session):
    ...
    tag = await create_tag(user.id, "TestTag", "blue", db_session)
...
async def test_get_user_tags(db_session):
    ...
    await create_tag(user.id, "A", "red", db_session)
    await create_tag(user.id, "B", "green", db_session)

    tags = await get_user_tags(user.id, db_session)
    assert len(tags) == 2
...
async def test_update_tag(db_session):
    ...
    updated = await update_tag(tag.id, user.id, "New", "blue", db_session)
...
async def test_soft_delete_tag(db_session):
    ...
    resp = await soft_delete_tag(tag.id, user.id, db_session)
...
async def test_add_and_remove_tag_from_file(db_session):
    ...
    db_session.add_all([user, file, tag])
    await db_session.commit()

    await add_tag_to_file(user.id, file.id, tag.id, db_session)
    ...
    await remove_tag_from_file(user.id, file.id, tag.id, db_session)
...
async def test_get_user_file_tags(db_session):
    ...
    tag2 = Tag(name="T2", color="green", user_id=user.id)
    db_session.add_all([user, file, tag1, tag2])
    await db_session.commit()

    await add_tag_to_file(user.id, file.id, tag1.id, db_session)
    await add_tag_to_file(user.id, file.id, tag2.id, db_session)

    tags = await get_user_file_tags(user.id, file.id, db_session)
