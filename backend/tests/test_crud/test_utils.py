...
def test_regression_uuid_validation_BTqJo9():
    """Regression test for UUID validation failure with 'BTqJo9'."""
    # This specific UUID was rejected by is_valid_public_uuid in CI
    # but was generated by generate_public_uuid, so it should be valid
    failing_uuid = "BTqJo9"
    assert is_valid_public_uuid(failing_uuid), f"UUID '{failing_uuid}' should be valid but was rejected"


def test_generated_uuids_always_pass_validation():
    """Test that all generated UUIDs pass validation."""
    # Generate multiple UUIDs and ensure they all pass validation
    for _ in range(100):
        generated_uuid = generate_public_uuid()
        assert is_valid_public_uuid(generated_uuid), f"Generated UUID '{generated_uuid}' should pass validation"
