...
<template>
  ...
  <div class="files-wrapper" :class="mode">
    ...
    <div class="files" :class="mode">
      <FilesItem v-for="(doc, idx) in userDocs.filter((doc) => !doc.filtered)" :class="{ active: activeIndex == idx }"
        :doc="doc" :mode="mode" @click="selectForPreview(doc, idx)" @dblclick="openRead(doc)" />
    </div>
  </div>
</template>
