...
<template>
  <div class="pane-header text-label" :class="mode">
    <template v-for="name in Object.keys(columnInfo)">
      <HeaderLabel v-if="shouldShow(name)" :name="name" v-model="columnState[name]"
        @sort="(mode) => handleColumnSortEvent(name, mode)" @filter="(tags) => handleColumnFilterEvent(name, tags)"
        :sortable="columnInfo[name]['sortable']" :filterable="columnInfo[name]['filterable']" />
    </template>
