...
describe('MultiSelectTags.vue', () => {
  ...
  const TagControlStub = defineComponent({
    ...
    setup(props, { emit, attrs }) {
      return () => h('div', { class: attrs.class, onClick: () => emit('update:modelValue', !props.modelValue) }, props.tag.name);
    },
  });

  const SeparatorStub = defineComponent({ name: 'Separator', setup() { return () => h('hr'); } });
  ...
  const TagStub = defineComponent({
    ...
    emits: ['rename'],
    setup(props, { emit, attrs }) {
      return () => h('div', { class: attrs.class, onClick: () => emit('rename', 'NewTag') }, props.tag.name);
    },
  });
  ...
  function mountMulti(tagsProp = [], fileProp = { id: 123 }) {
    return mount(MultiSelectTags, {
      ...
      global: {
        provide: { fileStore },
        stubs: { ContextMenu: ContextMenuStub, TagControl: TagControlStub, Separator: SeparatorStub, Tag: TagStub },
      },
    });
  }
...
it('updates internal selection state via v-model when TagControl emits update', async () => {
  const wrapper = mount(MultiSelectTags, {
    ...
    global: { provide: { fileStore }, stubs: { ContextMenu: ContextMenuStub, TagControl: TagControlStub, Separator: SeparatorStub, Tag: TagStub } },
  });
  ...
  it('creates a new tag on rename event on new-tag placeholder', async () => {
    ...
    const newTagStub = wrapper.findAllComponents(TagStub)
      .find(w => w.props('tag').id === null);
    await nextTick();
