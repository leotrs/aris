...
const ThemeSwitchStub = defineComponent({ name: 'ThemeSwitch', template: '<div class="theme-switch"/>' });
...
describe('FileSettings.vue', () => {
  ...
  it('updates background when ColorPicker emits change', async () => {
    const settings = { background: '', fontSize: '', lineHeight: '', fontFamily: '', marginWidth: '' };
  ...
  it('emits save event with current settings when save button is clicked', async () => {
    const settings = { background: 'b', fontSize: 'fs', lineHeight: 'lh', fontFamily: 'ff', marginWidth: 'mw' };
    const wrapper = mountFileSettings(settings);
    wrapper.vm.startReceivingUserInput();
