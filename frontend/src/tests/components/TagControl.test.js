...
describe('TagControl.vue', () => {
  ...
  it('updates tag color via ColorPicker change', async () => {
    ...
    expect(picker.props('colors')).toMatchObject({ red: 'var(--red-400)', green: 'var(--green-400)', purple: 'var(--purple-400)', orange: 'var(--orange-400)' });
  ...
  it('invokes startEditing on rename menu item click', async () => {
    const wrapper = mountControl();
    const renameItem = wrapper.findAllComponents(ContextMenuItemStub)
  ...
  it('deletes tag on delete menu item click', async () => {
    const wrapper = mountControl();
    const deleteItem = wrapper.findAllComponents(ContextMenuItemStub)
