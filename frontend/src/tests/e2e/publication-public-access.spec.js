...
test.describe('Publication to Public Access Workflow', () => {
  ...
  test('complete publication to public access workflow', async ({ page }) => {
    ...
    if (await publishButton.isVisible()) {
      ...
      await expect(page.locator('[data-testid="publication-success"]')).toBeVisible({ timeout: 10000 })
      ...
      await expect(page.locator('body')).toContainText('Testing end-to-end publication from creation to public access.')
  ...
  test('demo content publication simulation', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="demo-manuscript"]', { timeout: 10000 })
...
test.describe('Public Access Verification', () => {
  ...
  test('public access works without authentication', async ({ page }) => {
    ...
    const response = await page.waitForResponse(response => response.url().includes('/ication/test01'))
  ...
  test('public access handles non-existent files correctly', async ({ page }) => {
    ...
    const response = await page.waitForResponse(response => response.url().includes('/ication/nonexistent123'))
    expect(response.status()).toBe(404)
