...
test.describe("Demo Workspace Functionality", () => {
  ...
  test.describe("Sidebar Interactions", () => {
    ...
    test("source panel shows RSM markup", async ({ page }) => {
      ...
      const sourceButton = page.locator('[data-testid*="source"], button:has-text("source")').first();
    ...
    test("margins panel affects document layout", async ({ page }) => {
      ...
      const marginsButton = page.locator('[data-testid*="margin"], button:has-text("margin")').first();
      ...
      if (await marginsButton.count() > 0) {
        ...
        const marginControls = page.locator('[data-testid*="margin-control"], .margin-control, input[type="range"]');
    ...
    test("activity panel displays correctly", async ({ page }) => {
      ...
      const activityButton = page.locator('[data-testid*="activity"], button:has-text("activity")').first();
  ...
  test.describe("Editor Integration", () => {
    test("can open editor panel", async ({ page }) => {
      ...
      const editorButton = page.locator('button:has-text("editor"), [data-testid*="editor"]').first();
  ...
  test.describe("Focus Mode", () => {
    test("can toggle focus mode with 'c' key", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="manuscript-container"]')).toBeVisible({ timeout: 10000 });
