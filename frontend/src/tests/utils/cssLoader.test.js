...
describe("cssLoader", () => {
  ...
  describe("loadCSS", () => {
    it("creates a link element with correct attributes", async () => {
      ...
      expect(link.href).toBe(`${mockApi.defaults.baseURL}/design-assets/css/test.css`);
    ...
    it("constructs correct URLs for design assets", () => {
      ...
      const link = document.querySelector('link[rel="stylesheet"]');
      expect(link.href).toBe(`${mockApi.defaults.baseURL}/design-assets/css/typography.css`);
    });
  });
  ...
  describe("loadDesignAssets", () => {
    it("loads all design asset CSS files", async () => {
      ...
      expectedFiles.forEach((filename, index) => {
        expect(links[index].href).toBe(`${mockApi.defaults.baseURL}/design-assets/css/${filename}`);
      });
