...
<template>
  <div class="tag-control">
    <Tag ref="tagRef" :tag="tag" :active="state" :editable="true" @click="state = !state"
      @doneEditing="(newName) => renameTag(newName)" />
    <ContextMenu>
      <div class="colors">
        <span v-for="c in ['red', 'green', 'purple', 'orange']" :class="['color', c, tag.color == c ? 'active' : '']"
          @click.stop="setColor(c)" @dblclick.stop>
        </span>
      </div>
      <ContextMenuItem icon="Edit" caption="Rename" @click="
        console.log('here');
      tagRef?.startEditing();
      " @dblclick.stop />
