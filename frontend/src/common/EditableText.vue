...
<template>
  <span>
    <span v-if="!isEditing" class="editable" :class="textClass" @click="() => editOnClick && startEditing()">
      {{ text }}
    </span>
    <input v-else v-model="inputValue" :class="inputClass" @keydown="handleKeydown" @blur="saveChanges" @click.stop
      @dblclick.stop ref="inputRef" />
  </span>
</template>
