...
<template>
  <ContextMenu :icon="icon">
    <TagControl v-for="(tag, idx) in userTags" class="item" :tag="tag" :key="tag.id"
      v-model="state.tagIsAssigned[idx]" />
    <div class="new-tag-wrapper item">
      <Tag :tag="newTagPlaceholder" :active="false" @rename="(name) => createTag(name)" v-model="renaming"
        @click.stop="renaming = true" @dblclick.stop />
    </div>
  </ContextMenu>
</template>
